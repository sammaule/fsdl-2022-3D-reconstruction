...
https://github.com/sunset1995/HorizonNet/blob/master/misc/pano_lsd_align.py
...
def warpImageFast(im, XXdense, YYdense):
    ...
    im = im[
        int(round(minY - 1)) : int(round(maxY)), int(round(minX - 1)) : int(round(maxX))
    ]
...
def rotatePanorama(img, vp=None, R=None):
    ...
    imgNew[-1, 1 : sphereW // 2 + 1, :] = img[
        -1, sphereW - 1 : sphereW // 2 - 1 : -1, :
    ]
    ...
    for i in range(numLine):
        ...
        ori_lines[i, 6] = np.arccos(
            (
                ...
                / (np.linalg.norm(coord1) * np.linalg.norm(coord2))
            ).clip(-1, 1)
        )
    ...
    for _ in range(3):
        ...
        for i in range(numLine):
            ...
            for j in np.nonzero(valid_curr)[0]:
                ...
                scr = (lines[i, 6] * lines[i, 7] + lines[j, 6] * lines[j, 7]) / (
                    lines[i, 6] + lines[j, 6]
                )
...
def icosahedron2sphere(level):
    ...
    for _ in range(level):
        ...
        # extrude
        coor = list(
            coor / np.tile(np.sqrt(np.sum(coor * coor, 1, keepdims=True)), (1, 3))
        )
...
def sphereHoughVote(
    ...
    for j in range(len(checkIDs1)):
        ...
        for i in range(len(checkIDs2)):
            ...
            cpv = np.cross(voteBinPoints[checkID1], voteBinPoints[checkID2]).reshape(
                1, 3
            )
            cpn = np.linalg.norm(cpv)
            dotProduct = (voteBinPoints * cpv).sum(1) / cpn
            checkIDs3 = np.nonzero(
                np.abs(dotProduct) > np.cos(orthTolerance * np.pi / 180)
            )[0]
...
def findMainDirectionEMA(lines):
    ...
    ang = (
        np.arccos((candiSet[tri[0, 0]] * candiSet[tri[0, 1]]).sum().clip(-1, 1))
        / np.pi
        * 180
    )
    binRadius = ang / 2
    initXYZ, score, angle = sphereHoughVote(
        segNormal, segLength, segScores, 2 * binRadius, 2, candiSet
    )
    ...
    curXYZ = initXYZ.copy()
    tol = np.linspace(
        4 * binRadius, 4 * binRadiusD, iter_max
    ...
    for it in range(iter_max):
        ...
        tcurXYZ, _, _ = sphereHoughVote(
            subSegNormal, subSegLength, subSegScores, 2 * binRadiusD, 2, subCandiSet
        )
...
def paintParameterLine(parameterLine, width, height):
    ...
    for i in range(len(lines)):
        ...
        uv = xyz2uvN(xyz, 1)
        m = np.minimum(
            np.floor((uv[:, 0] + np.pi) / (2 * np.pi) * width) + 1, width
        ).astype(np.int32)
        n = np.minimum(
            np.floor(((np.pi / 2) - uv[:, 1]) / np.pi * height) + 1, height
        ).astype(np.int32)
        panoEdgeC[n - 1, m - 1] = i
