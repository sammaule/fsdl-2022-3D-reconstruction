...
class GlobalHeightStage(nn.Module):
    ...
    def forward(self, conv_list: List[torch.Tensor], out_w: int):
        ...
        feature = torch.cat(
            [
                f(conv_list[i], out_w).reshape(bs, -1, out_w)
...
class HorizonNet(nn.Module):
    ...
    def __init__(self, backbone, use_rnn):
        ...
        self.register_buffer(
            "x_mean",
            torch.FloatTensor(np.array([0.485, 0.456, 0.406])[None, :, None, None]),
            persistent=True,
        )
        self.register_buffer(
            "x_std",
            torch.FloatTensor(np.array([0.229, 0.224, 0.225])[None, :, None, None]),
            persistent=True,
        )
        ...
        if self.use_rnn:
            ...
            self.linear = nn.Linear(
                in_features=2 * self.rnn_hidden_size, out_features=3 * self.step_cols
            )
            ...
            self.linear[-1].bias.data[1 * self.step_cols : 2 * self.step_cols].fill_(
                -0.478
            )
    ...
    def forward(self, x):
        ...
        if self.use_rnn:
            ...
            output, hidden = self.bi_rnn(
                feature
            )  # [seq_len, b, num_directions * hidden_size]
            output = self.drop_out(output)
            output = self.linear(output)  # [seq_len, b, 3 * step_cols]
            output = output.view(
                output.shape[0], output.shape[1], 3, self.step_cols
            )  # [seq_len, b, 3, step_cols]
            output = output.permute(1, 2, 0, 3)  # [b, 3, seq_len, step_cols]
            output = output.contiguous().view(
                output.shape[0], 3, -1
            )  # [b, 3, seq_len*step_cols]
        else:
