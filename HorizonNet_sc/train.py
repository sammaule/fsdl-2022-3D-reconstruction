...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--id", required=True, help="experiment id to name checkpoints and logs"
    )
    ...
    parser.add_argument(
        "--pth", default=None, help="path to load saved checkpoint." "(finetuning)"
    )
    # Model related
    parser.add_argument(
        "--backbone",
    ...
    parser.add_argument(
        "--no_rnn", action="store_true", help="whether to remove rnn or not"
    )
    ...
    parser.add_argument(
        "--valid_root_dir",
        default="data/layoutnet_dataset/valid",
        help="root directory to validation dataset. "
        "should contains img, label_cor subdirectories",
    )
    parser.add_argument(
        "--no_flip", action="store_true", help="disable left-right flip augmentation"
    )
    parser.add_argument(
        "--no_rotate",
        action="store_true",
    ...
    parser.add_argument(
        "--no_gamma", action="store_true", help="disable gamma augmentation"
    )
    parser.add_argument(
        "--no_pano_stretch", action="store_true", help="disable pano stretch"
    )
    parser.add_argument(
        "--num_workers", default=6, type=int, help="numbers of workers for dataloaders"
    )
    # optimization related arguments
    parser.add_argument("--freeze_earlier_blocks", default=-1, type=int)
    parser.add_argument(
        "--batch_size_train", default=4, type=int, help="training mini-batch size"
    )
    parser.add_argument(
        "--batch_size_valid", default=2, type=int, help="validation mini-batch size"
    )
    parser.add_argument("--epochs", default=300, type=int, help="epochs to train")
    parser.add_argument(
        "--optim", default="Adam", help="optimizer to use. only support SGD and Adam"
    )
    parser.add_argument("--lr", default=1e-4, type=float, help="learning rate")
    parser.add_argument(
        "--lr_pow", default=0.9, type=float, help="power in poly to drop LR"
    )
    parser.add_argument(
        "--warmup_lr",
        default=1e-6,
    ...
    parser.add_argument(
        "--warmup_epochs", default=0, type=int, help="numbers of warmup epochs"
    )
    parser.add_argument(
        "--beta1", default=0.9, type=float, help="momentum for sgd, beta1 for adam"
    )
    parser.add_argument(
        "--weight_decay", default=0, type=float, help="factor for L2 regularization"
    )
    ...
    parser.add_argument(
        "--disp_iter", type=int, default=1, help="iterations frequency to display"
    )
    parser.add_argument(
        "--save_every", type=int, default=25, help="epochs frequency to save state_dict"
    )
    ...
    for ith_epoch in trange(1, args.epochs + 1, desc="Epoch", unit="ep"):
        ...
        if args.valid_root_dir:
            valid_loss = {}
            for jth in trange(
                len(dataset_valid), desc="Valid ep%d" % ith_epoch, position=2
            ):
                ...
                with torch.no_grad():
                    ...
                    losses["delta_1"] = torch.FloatTensor(
                        [true_eval["overall"]["delta_1"]]
                    )
            ...
            print(
                "Ep%3d %.4f vs. Best %.4f"
                % (ith_epoch, now_valid_score, args.best_valid_score)
            )
            if now_valid_score > args.best_valid_score:
                args.best_valid_score = now_valid_score
                save_model(
                    net, os.path.join(args.ckpt, args.id, "best_valid.pth"), args
                )

        # Periodically save model
        if ith_epoch % args.save_every == 0:
            save_model(
                net, os.path.join(args.ckpt, args.id, "epoch_%d.pth" % ith_epoch), args
            )
