...
with ZipFile(args.zippath) as zipf:
    ...
    for scene_id, image_id in tqdm(id_set):
        path_img = path_format % (
            scene_id,
            image_id,
            "%s/rgb_rawlight.png" % args.style,
        )
        ...
        with zipf.open(path_img) as f:
            ...
            imageio.imwrite(
                os.path.join(
                    args.outdir, scene_id, "rgb", image_id + "_rgb_rawlight.png"
                ),
                rgb,
            )
        with zipf.open(path_layout) as f:
            with open(
                os.path.join(args.outdir, scene_id, "layout", image_id + "_layout.txt"),
                "w",
            ) as fo:
