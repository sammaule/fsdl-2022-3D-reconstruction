...
parser.add_argument("--root", required=True, help="path to the dataset directory")
parser.add_argument("--train_txt", required=True, help="path to save txt for train")
parser.add_argument("--valid_txt", required=True, help="path to save txt for valid")
parser.add_argument("--test_txt", required=True, help="path to save txt for test")
...
def gen_pairs(scene_id_lst):
    ...
    for scene_id in scene_id_lst:
        for fname in os.listdir(os.path.join(args.root, scene_id, "rgb")):
            ...
            layout_k = os.path.join(
                os.path.join(scene_id, "layout", room_id + "_layout.txt")
            )
