...
def inference(
    ...
    if visualize:
        vis_out = visualize_a_data(
            x[0], torch.FloatTensor(y_bon_[0]), torch.FloatTensor(y_cor_[0])
        )
    else:
        vis_out = None
...
if __name__ == "__main__":
    ...
    parser.add_argument("--pth", required=True, help="path to load saved checkpoint.")
    parser.add_argument(
        "--img_glob",
        required=True,
    ...
    parser.add_argument(
        "--flip",
        action="store_true",
        help="whether to perfome left-right flip. " "# of input x2.",
    )
    parser.add_argument(
        "--rotate",
        nargs="*",
    ...
    net = utils.load_trained_model(HorizonNet, args.pth)
    # .to(device)
    net.eval()
    ...
    with torch.no_grad():
        for i_path in tqdm(paths, desc="Inferencing"):
            ...
            if vis_out is not None:
                ...
                Image.fromarray(vis_out).resize((vw // 2, vh // 2), Image.LANCZOS).save(
                    vis_path
                )
