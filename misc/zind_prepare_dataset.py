...
def show_statistic(root, scene_id_lst):
    ...
    for scene_id in scene_id_lst:
        ...
        for floor_id, partial_room_id, pano_id, pano_data in label_iterator(label):
            sta.append(
                [
                    pano_data["is_primary"],
                    pano_data["is_inside"],
                    pano_data.get("is_ceiling_flat", False),
                ]
            )
    sta = np.array(sta)
    ...
    print(
        f"is_primary                                : {is_primary.sum()} ({is_primary.mean()*100:.1f}%)"
    )
    print(
        f"is_inside                                 : {is_inside.sum()} ({is_inside.mean()*100:.1f}%)"
    )
    print(
        f"is_ceiling_flat                           : {is_ceiling_flat.sum()} ({is_ceiling_flat.mean()*100:.1f}%)"
    )
    print(
        f"is_inside       given that is_primary     : {is_inside[is_primary].sum()} ({is_inside[is_primary].mean()*100:.1f}%)"
    )
    print(
        f"is_ceiling_flat given that is_primary     : {is_ceiling_flat[is_primary].sum()} ({is_ceiling_flat[is_primary].mean()*100:.1f}%)"
    )
...
def run(scene_id, split, args):
    ...
    for floor_id, partial_room_id, pano_id, pano_data in label_iterator(label):
        ...
        if args.is_ceiling_flat != -1:
            if (
                args.is_ceiling_flat == 0 and pano_data.get("is_ceiling_flat", False)
            ) or (
                args.is_ceiling_flat == 1
                and not pano_data.get("is_ceiling_flat", False)
            ):
                continue
        ...
        img.save(
            os.path.join(args.outdir, split, "img", f"{scene_id}_{key}.jpg"),
            quality=95,
            optimize=True,
        )
        with open(
            os.path.join(args.outdir, split, "label_cor", f"{scene_id}_{key}.txt"), "w"
        ) as f:
